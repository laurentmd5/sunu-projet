...
CREATE TABLE `ProjectUser` (
    ...
    `userId` VARCHAR(36) NOT NULL,
    `projectId` VARCHAR(36) NOT NULL,
    `role` ENUM('OWNER', 'MANAGER', 'VIEWER', 'MEMBER') NOT NULL DEFAULT 'MEMBER',
    `scope` ENUM('INTERNAL', 'EXTERNAL') NOT NULL DEFAULT 'INTERNAL',
    `inviteToken` VARCHAR(100) NULL,

    UNIQUE INDEX `ProjectUser_inviteToken_key`(`inviteToken`),
    INDEX `ProjectUser_projectId_role_idx`(`projectId`, `role`),
...
CREATE TABLE `TeamMeeting` (
    ...
    `provider` ENUM('NONE', 'JITSI') NOT NULL DEFAULT 'NONE',
